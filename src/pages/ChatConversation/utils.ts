...
export const getSystemMessage = (options: {
  ...
  if (isConnectionDeleted) {
    return buildSystemMessage({ kind: 'deleted', text: t('chatConversation.connectionDeleted') })
  }

  if (isConnectionTerminated) {
    return buildSystemMessage({ kind: 'terminated', text: t('chatConversation.connectionTerminated') })
  }

  if (isConnectionBlocked) {
    return buildSystemMessage({ kind: 'blocked', text: t('chatConversation.blockedThisConnection') })
  }

  if (!isConnectionCompleted) {
    return buildSystemMessage({
      kind: 'pending',
      text: t('chatConversation.waitingAcceptInvitation', { name: displayName }),
    })
  }
