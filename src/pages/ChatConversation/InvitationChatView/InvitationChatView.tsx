...
const InvitationChatView = ({ associatedRecordId: outOfBandId, metadata, role, agent }: Props) => {
  ...
  const invitationType = t(
    isService(did)
      ? 'chatConversation.invitationRequestService'
      : 'chatConversation.invitationRequestSubConnection',
  )
  ...
  const goToExistingConnection = async () => {
    ...
    if (connection) {
      ...
      toast({ type: 'error', message: `${t('chatConversation.invitationConnectionNotFound')}` })
    }
  }
    ...
    [InvitationState.Accepted]: (
      <View style={styles.acceptedContainer}>
        <Text fontFamily="EuclidCircularA-Bold" style={styles.acceptedText}>
          {t('chatConversation.acceptedInvitation')}
        </Text>
      </View>
    ),
    [InvitationState.AlreadyConnected]: (
      <BlueButton
        text={t('chatConversation.alreadyConnected')}
    ...
    <>
      ...
      <View style={styles.containerMain}>
        ...
        <Text style={styles.subTitle}>
          {isReceiver
            ? t('chatConversation.invitationDescription')
            : t('chatConversation.sentInvitationDescription')}
          <Text fontFamily="EuclidCircularA-SemiBold" style={styles.textSemiBold}>
            {' '}
            {serviceInfo?.name ?? label}
          </Text>
          {!isService && t('chatConversation.asASubConnectionOf')}
          {!isService && (
            <Text fontFamily="EuclidCircularA-SemiBold" style={styles.textSemiBold}>
              {' '}
              {chatThread?.topic}
