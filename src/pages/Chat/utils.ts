...
export const getSystemMessage = (options: {
  ...
  if (isConnectionTerminated) {
    return buildSystemMessage({ kind: 'terminated', text: t('chat.connectionTerminated') })
  }

  if (isConnectionBlocked) {
    return buildSystemMessage({ kind: 'blocked', text: t('chat.blockedThisConnection') })
  }

  if (!isConnectionCompleted) {
    return buildSystemMessage({
      kind: 'pending',
      text: t('chat.waitingAcceptInvitation', { name: displayName }),
    })
  }
