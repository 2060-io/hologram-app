...
const parse2LineMRZ = (firstRow: string, secondRow: string) => {
  ...
  let namesFromLine: { givenNames: string[]; lastNames: string[] } = extractNamesFromLine(5, firstRow)
 ...
 */
const parse3LineMRZ = (firstRow: string, secondRow: string, thirdRow: string) => {
  let docType = extractDocType(firstRow)
  let namesFromLine: { givenNames: string[]; lastNames: string[] } = extractNamesFromLine(0, thirdRow)
  let givenNames: string[] = namesFromLine.givenNames
  let lastNames: string[] = namesFromLine.lastNames
...
export const getDocTypeFromCode = (docCode?: string) => {
  return ListItemData.DOCUMENT_TYPE.find(doc => doc.codes.find(code => code === docCode))?.value
}
 ...
 */
const extractIdNumber = (line: string, startingIndex: number, endingIndex: number) => {
 ...
 */
const extractCountry = (line: string, startingIndex: number, endingIndex: number) => {
  ...
  return countryIsoJson.find((item: CountryIsoCode) => item.isoCountryCode === country)?.isoCountryCode
}
 ...
 */
const extractDateOfExpirationFromLine = (startingIndex: number, line: string) => {
  // replace all 'O' with '0'
  ...
  let expDateCheckSum = checkSum(
    twoDigitYearOfExpiration + twoDigitMonthOfExpiration + twoDigitDayOfExpiration,
  )
