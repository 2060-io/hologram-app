...
- backup: Encrypted string value for backup password that user set when builds backup and need to provide when restores this wallet backup
...
/data/user/0/io.twentysixty.mobileagent.dev/files or
...
3. **./files/media** directory: Refers to directory where all media content are going to live. Inside it there is a subdirectory called "previews" where you know thumbnails for videos and images in lower resolution are stores
4. **./files/wallet** directory: Refers to DB files are stored. Realm and afj sqlite wallet files
5. **./files/config.json**: Refers to explained config file above
